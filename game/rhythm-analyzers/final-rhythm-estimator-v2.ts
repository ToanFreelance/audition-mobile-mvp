...
function detectedQuality(row: RhythmEngineResult) {
  ...
  const jitterQuality = clamp(1 - Math.max(0, jitter - 4) / 60, 0.2, 1);
  const span = (row.beatTimesMs[row.beatTimesMs.length - 1] ?? 0) - (row.beatTimesMs[0] ?? 0);
  const spanQuality = clamp(span / 150_000, 0.3, 1);
  const modeQuality = row.tempoMode === "CONSTANT" ? 1 : row.tempoMode === "UNKNOWN" ? 0.72 : 0.4;
...
function addExactObservation(target: ExactObservation[], row: RhythmEngineResult, rawBpm: number | null, targetBpm: number, weight: number, label: string) {
...
function acceptedBeatrootFitObservation(rows: RhythmEngineResult[], targetBpm: number): ExactObservation | null {
  ...
  return {
    family: "music-tempo",
    bpm: mapped.mapped,
    weight: 1.15 * clamp(1 - raw.rmseMs / 24, 0.3, 1),
...
function buildFamilyEstimates(packageRows: RhythmEngineResult[], targetBpm: number) {
  ...
  for (const row of packageRows) {
    if (row.kind !== "package" || row.error) continue;
    addExactObservation(observations, row, row.bpm, targetBpm, 0.42, `${row.engine} tempo`);
    const quality = detectedQuality(row);
    if (quality <= 0) continue;
    addExactObservation(observations, row, row.derivedBpmFromIntervals, targetBpm, 0.62 * quality, `${row.engine} interval median`);
    addExactObservation(observations, row, stableLocalMedian(row), targetBpm, 0.74 * quality, `${row.engine} local median`);
    addExactObservation(observations, row, longLagBpm(row.beatTimesMs, 16), targetBpm, 0.78 * quality, `${row.engine} lag16`);
    addExactObservation(observations, row, longLagBpm(row.beatTimesMs, 32), targetBpm, 0.92 * quality, `${row.engine} lag32`);
    addExactObservation(observations, row, longLagBpm(row.beatTimesMs, 64), targetBpm, 1.05 * quality, `${row.engine} lag64`);
    const longSpan = rowRaw(row).longSpanBpm;
    addExactObservation(observations, row, finite(longSpan) ? longSpan : null, targetBpm, 1.0 * quality, `${row.engine} long-span`);
  }
  ...
  for (const [family, items] of grouped) {
    ...
    const evidenceWeight = clamp(items.reduce((sum, item) => sum + item.weight, 0) / 3.5, 0.45, 1);
    const spreadQuality = 1 / (1 + spread / 0.18);
    estimates.push({
      family,
      bpm: center,
      spreadBpm: spread,
      weight: evidenceWeight * (0.65 + 0.35 * spreadQuality),
      observations: items,
    });
  }
  return estimates;
...
function robustFamilyCenter(estimates: FamilyEstimate[]) {
  ...
  const initial = weightedMedian(estimates);
  if (initial == null) return { center: null, mad: null, retained: [] };
  const initialMad = weightedMad(estimates, initial) ?? 0;
  const threshold = Math.max(0.10, initialMad * 3.5);
  const retained = estimates.filter(item => Math.abs(item.bpm - initial) <= threshold);
  const source = retained.length >= 2 ? retained : estimates;
...
function fuseExactAudioBpm(
  ...
  const fusion: ExactObservation[] = family.retained.map(item => ({
    ...
    bpm: item.bpm,
    weight: item.weight,
    label: `${item.family} family center`,
  }));
  const onset = nearestValidator(onsetCandidates, targetBpm);
  const phase = nearestValidator(phaseCandidates, targetBpm);
  if (onset) fusion.push({ family: "ONSET GRID", bpm: onset.bpm, weight: 0.52 * clamp(onset.score, 0, 1), label: "ONSET GRID" });
  if (phase) {
    const driftQuality = phase.driftMsPerSecond == null ? 0.6 : 1 / (1 + Math.abs(phase.driftMsPerSecond) / 0.35);
    fusion.push({ family: "PHASE GRID", bpm: phase.bpm, weight: 0.82 * clamp(phase.score, 0, 1) * (0.65 + 0.35 * driftQuality), label: "PHASE GRID" });
  }
...
function findNominalEvidence(
  ...
  for (const row of packageRows) {
    ...
    const local = stableLocalMedian(row);
    const intervalNear = mappedValueNearInteger(row.derivedBpmFromIntervals, row, targetBpm, integerBpm, 0.035);
    const localNear = mappedValueNearInteger(local, row, targetBpm, integerBpm, 0.035);
    ...
    const stableWindows = substantialLocalWindows(row).filter(window =>
      window.bpm != null && Math.abs(window.bpm * mapped.value - integerBpm) <= 0.05,
    ).length;
    if (stableWindows >= 4) quantizedFamilies.add(row.engine);
  }

  const onsetNear = onsetCandidates.some(candidate => Math.abs(candidate.bpm - integerBpm) <= 0.055);
  const phaseNear = phaseCandidates.some(candidate => Math.abs(candidate.bpm - integerBpm) <= 0.055);
  const familyNear = familyEstimates.filter(item => Math.abs(item.bpm - integerBpm) <= 0.08).map(item => item.family);
  const validatorAgreement = onsetNear && phaseNear && new Set(familyNear).size >= 2;
  const reasons: string[] = [];
  let confidence = 0;
  if (validatorAgreement) {
    confidence = 0.92;
    reasons.push(`ONSET+PHASE both resolve near ${integerBpm} BPM and ${new Set(familyNear).size} package families agree`);
  }
  if (quantizedFamilies.size) {
    confidence = Math.max(confidence, quantizedFamilies.size >= 2 ? 0.88 : 0.70);
    reasons.push(`${[...quantizedFamilies].join(", ")} detected grid has integer-spaced interval + local-window evidence`);
  }
  if (!reasons.length) return null;
  return { bpm: integerBpm, confidence, reasons, quantizedFamilies: [...quantizedFamilies], validatorAgreement };
}
...
function exactTempoConfidence(familyEstimates: FamilyEstimate[], familyMad: number, durationMs: number, audioBpm: number, nominal: NominalEvidence | null) {
  const countQuality = clamp(familyEstimates.length / 4, 0, 1);
  const spreadQuality = 1 / (1 + familyMad / 0.06);
  const robust95Bpm = Math.max(0.001, familyMad * 2.5);
  const endDrift95Ms = durationMs > 0 && audioBpm > 0 ? durationMs * robust95Bpm / audioBpm : 999;
  const driftQuality = 1 / (1 + endDrift95Ms / 45);
  const nominalBoost = nominal?.confidence && Math.abs(audioBpm - nominal.bpm) <= Math.max(0.01, audioBpm * DRIFT_BUDGET_MS / Math.max(durationMs, 1))
    ? nominal.confidence * 0.16
    : 0;
  return {
    confidence: clamp(countQuality * 0.28 + spreadQuality * 0.38 + driftQuality * 0.34 + nominalBoost, 0, 1),
    robust95Bpm,
    endDrift95Ms,
  };
}
...
function sourceSpeedStatus(audioBpm: number, nominal: NominalEvidence | null, snapToleranceBpm: number) {
  ...
  if (delta <= 0.08) return "REVIEW" as const;
...
export async function runFinalRhythmEstimator(
  ...
  try {
    progress?.("FINAL RHYTHM v2 · calibrating metrical level, independent families, drift and gameplay anchor…");
    ...
    const nominal = findNominalEvidence(packageRows, familyEstimates, onsetCandidates, phaseCandidates, top.bpm, audioPulseBpm);
    ...
    const safeIntegerSnap = Boolean(nominal && Math.abs(audioPulseBpm - nominal.bpm) <= snapToleranceBpm);
    ...
    const endDriftToNominalMs = nominal ? durationMs * (audioPulseBpm / nominal.bpm - 1) : null;
    ...
    const exactConf = exactTempoConfidence(exact.retainedFamilies, exact.familyMad, durationMs, audioPulseBpm, nominal);
    ...
    let overallConfidence = clamp(metricalConf * 0.36 + exactConf.confidence * 0.34 + phaseConf * 0.18 + gameplayAnchorConf * 0.12, 0, 1);
    if (variable) overallConfidence *= 0.62;
    if (speedStatus === "LIKELY_SOURCE_SPEED_MISMATCH") overallConfidence *= 0.90;
    ...
    const familySummary = familyEstimates
      ...
      .map(item => `${item.family}=${item.bpm.toFixed(4)}±${item.spreadBpm.toFixed(4)}`)
      .join(" · ");
    ...
    const nominalNote = nominal
      ? `Nominal candidate=${nominal.bpm.toFixed(0)} BPM (${Math.round(nominal.confidence * 100)}%): ${nominal.reasons.join("; ")}. Source-speed status=${speedStatus}; playbackRate-to-nominal=${playbackRateToNominal?.toFixed(6)}; nominal end-drift if unnormalized=${endDriftToNominalMs?.toFixed(1)}ms.`
      : "No strong intended-integer/nominal tempo evidence; decimal source tempo is retained.";
    const anchorNote = authoredAnchor != null
      ? `BPM selection and audio-phase validation do NOT use authored Space Start. The returned GAMEPLAY grid is intentionally phase-locked to authored SPACE #1=${authoredAnchor.toFixed(1)}ms so the benchmark/live gauge cannot silently replace the chart's Perfect anchor.`
    ...
    const normalized = normalizeResult({
      ...
      engine: "FINAL RHYTHM",
      variant: `${variable ? "VARIABLE REVIEW" : "v2 calibrated"} · ${status}`,
      version: VERSION,
      kind: "custom",
      ...
      confidenceScale: "Internal calibrated confidence. Separate metrical/exact-tempo/audio-phase/gameplay-anchor components are exposed in raw; package rows are collapsed to one exact estimate per independent engine family before fusion.",
      beatTimesMs,
      onsetTimesMs: autoGrid.onsetTimesMs ?? phaseGrid.onsetTimesMs,
      processingTimeMs: performance.now() - started,
      notes: `FINAL RHYTHM v2. Selected metrical cluster=${top.bpm.toFixed(4)} BPM; measured audioPulseBpm=${audioPulseBpm.toFixed(6)}; gameplayBpm=${gameplayBpm.toFixed(6)}${safeIntegerSnap ? " (safe intended-integer snap within 30ms end-of-track drift budget)" : ""}. Independent-family exact evidence: ${familySummary || "n/a"}. Approx robust 95% family-spread envelope=±${exactConf.robust95Bpm.toFixed(4)} BPM => ${exactConf.endDrift95Ms.toFixed(1)}ms end-of-track uncertainty. Confidence components: metrical=${Math.round(metricalConf * 100)}%, exactTempo=${Math.round(exactConf.confidence * 100)}%, audioPhase=${Math.round(phaseConf * 100)}%, gameplayAnchor=${Math.round(gameplayAnchorConf * 100)}%. ${nominalNote} ${anchorNote} Candidate ranking: ${ranking}. ${variable ? "Multiple independent detected-grid families report variable tempo: do not treat the constant BPM grid as production truth; preserve detected beatTimes[]/human review." : "Tempo evidence is consistent with a constant source grid."}`,
      raw: {
        version: VERSION,
        status,
        variable,
    ...
    return normalizeResult({
      ...
      beatTimesMs: [],
      processingTimeMs: performance.now() - started,
      error: error instanceof Error ? error.message : "Final rhythm estimator v2 failed",
      notes: "FINAL RHYTHM v2 failed independently. Package, ONSET GRID and PHASE GRID diagnostics remain available.",
    }, input);
  }
}
