...
export class RhythmRuntime {
  ...
  private commandCursor = 0; private timingMoveIndex = 0; private started = false; private finished = false; private raf = 0; private lastStatsSignature = "";
  ...
  start() { this.stop(); this.engine = new RhythmEngine(this.chart); this.clock = new BeatClock(this.chart.bpm, this.chart.offsetMs); this.commandCursor = 0; this.timingMoveIndex = 0; this.started = true; this.finished = false; this.lastStatsSignature = ""; this.clock.start(); this.emitSequence(); this.emitStats(true); this.loop(); }
  ...
  get timingGaugePercent() { if (!this.started) return 0; const cycleBeat = this.clock.currentBeat % GAUGE_DURATION_BEATS; return (cycleBeat / GAUGE_DURATION_BEATS) * 100; }
  ...
  private advanceAfterJudgement(judgement: Judgement) { this.timingMoveIndex += 1; this.commandCursor = 0; this.callbacks.onJudgement?.(judgement); this.callbacks.onPulse?.(); this.emitStats(true); this.emitSequence(); if (this.timingMoveIndex >= this.totalMoves) { this.finished = true; this.callbacks.onFinished?.(cloneStats(this.engine.stats)); } }
