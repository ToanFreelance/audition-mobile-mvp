...
export default function AuditionGame() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (transient.actionId) {
        ...
        window.setTimeout(() => { actionRef.current = null; }, 720);
      }
      if (transient.wrongDirection) window.setTimeout(() => setWrong(null), 180);
      if (snapshot.phase === "finished") setStarted(false);
  ...
  const currentBeat = () => {
    ...
    return elapsed * chart.bpm / 60;
  };

  const startGame = async () => {
    const engine = engineRef.current ?? new AuditionEngine(chart);
    engineRef.current = engine; engine.start(); fallbackStartRef.current = performance.now();
    setStats(EMPTY_STATS); setJudgement(null); setWrong(null); setStarted(true); setPhase("input"); setTurn(chart.turns[0]); setSequence(chart.turns[0].directions); setCompleted(0); setGauge(0); setAudioError(false);
    if (audioRef.current && sound) {
      try { audioRef.current.currentTime = 0; await audioRef.current.play(); } catch { setAudioError(true); }
    }
  };
...
function Arrow({ direction }: { direction: Direction }) { const rotation = direction === "right" ? 0 : direction === "down" ? 90 : direction === "left" ? 180 : 270; return <svg viewBox="0 0 42 40" aria-hidden="true" style={{transform:`rotate(${rotation}deg)`}}><path d="M5 14H22V8L36 20L22 32V26H5V14Z"/></svg>; }
