...
import type { ArrowToken, Direction, GameStats, Judgement } from "../game/types";
...
export default function GameShell() {
  ...
  const runtime = useMemo(() => new RhythmRuntime(activeChart, {
    onStats: setStats,
    onArrowCommand: (next, filled) => { setArrowCommand(next); setCompleted(filled); },
    onLevel: setLevel,
    onCountdown: (value) => {
      setCountdown(value);
      if (value === 0) {
  ...
  const startGame = useCallback(async () => {
    ...
    try {
      transport.reset();
      runtime.setTimeSource(() => transport.getCurrentTimeMs());
      runtime.setSongDuration(transport.durationMs || selectedMusic.durationMs);
      runtime.start();
      setAudioState("loading");
      await transport.play();
      setAudioState("playing");
  ...
  const pressDirection = useCallback((direction: Direction) => {
    ...
    const target = arrowCommand[completed]?.requiredDirection;
  ...
  }, [runtime, arrowCommand, completed, started]);
  ...
  const firstPerfectMs = activeChart.firstPerfectMs ?? 15000;
  const firstPerfectSeconds = firstPerfectMs / 1000;
  const firstAnchorDeltaMs = songTime * 1000 - firstPerfectMs;
  ...
  const anchorNear = started && Math.abs(firstAnchorDeltaMs) <= 90;
    ...
    <main className="audition-page">
      ...
      <section className="audition-stage">
        ...
        <div className="audition-hud">
          ...
          <div className="top-actions portrait-top-actions"><button type="button" onClick={() => setMenuOpen(true)} aria-label="Mở menu"><span aria-hidden="true">Ⅱ</span><small>MENU</small></button></div>
          ...
          <div className={`command-zone ${showCommandStrip ? "visible" : "pre-intro"}`}>
            ...
            <div className="command-strip" style={{ background: "transparent", backgroundImage: "none", WebkitBackdropFilter: "none", backdropFilter: "none" }}>{arrowCommand.map((token, index) => { const isCompleted = index < completed; const direction = renderedArrowDirection(token, isCompleted); const isTarget = index === completed; const isWrong = isTarget && wrongDirection !== null && wrongDirection !== token.requiredDirection; const visualState = isCompleted ? "completed" : token.reverse ? "reverse" : "normal"; return <div data-direction={direction} data-reverse={token.reverse} aria-label={`${token.reverse ? "reverse " : ""}${direction}`} key={`${runtime.currentTurn?.absoluteTurn}-${index}-${direction}`} className={`command-key ${isCompleted ? "done" : ""} ${token.reverse ? "reverse" : ""} ${isTarget ? "target" : ""} ${isWrong ? "wrong" : ""}`}><CommandTokenVisual direction={direction} state={visualState} target={isTarget} visualId={`${runtime.currentTurn?.absoluteTurn ?? "idle"}-${index}`} /></div>; })}</div>
            ...
            <div className="gauge-readout" style={{ display: debugEnabled ? "flex" : "none", flexWrap: "wrap", justifyContent: "center", gap: "3px 10px", lineHeight: 1.2 }}>
              ...
              <span>Δ1 {firstAnchorDeltaMs >= 0 ? "+" : ""}{firstAnchorDeltaMs.toFixed(0)}ms</span>
