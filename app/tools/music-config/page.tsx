...
export default function MusicConfigPage() {
  ...
  return <main className={`music-config-page theme-${theme} ${dockExpanded ? "audio-dock-expanded" : "audio-dock-collapsed"}`} style={pageStyle}>
    ...
    <footer ref={stickyActionsRef} className="sticky-actions"><div className="sticky-status"><strong>{config.title || "Untitled track"}</strong><span>{message || "Listen for Beat-4, then USE CURRENT. Nearby analyzed Beat-4 anchors snap automatically."}</span></div><div className="sticky-action-group"><button className="button button-primary sticky-use-current" onClick={() => { const rawMs = Math.max(0, Math.min(audioDurationMs, Math.round(currentTimeMs))); const nearestAnchor = anchors.length ? anchors.reduce((best, anchor) => Math.abs(anchor.ms - rawMs) < Math.abs(best.ms - rawMs) ? anchor : best, anchors[0]) : undefined; const beatMs = timingBpm > 0 ? 60000 / timingBpm : Number.POSITIVE_INFINITY; const snapToleranceMs = Math.max(120, beatMs * 0.55); const snapped = Boolean(nearestAnchor && Math.abs(nearestAnchor.ms - rawMs) <= snapToleranceMs); const ms = snapped && nearestAnchor ? nearestAnchor.ms : rawMs; patch("spaceStartMs", ms); if (snapped && nearestAnchor) patch("spaceStartBeat", nearestAnchor.beatIndex); setSelectedAnchorMs(ms); setUsedAnchorMs(ms); setPreviewingAnchorMs(null); if (snapped && nearestAnchor) { waveformRef.current?.seekTo(ms); setCurrentTimeMs(ms); const delta = nearestAnchor.ms - rawMs; setMessage(`✓ Heard ${formatTime(rawMs, 3)} → snapped ${delta >= 0 ? "+" : ""}${delta}ms to Beat ${nearestAnchor.beatIndex}: ${formatTime(ms, 3)}.`); } else { setMessage(`✓ Current position applied as Space Start: ${formatTime(ms, 3)}${anchors.length ? " (outside Beat-4 snap window)." : "."}`); } }} type="button">USE CURRENT</button>{isExistingChart && <button className="button button-danger" disabled={saving} onClick={() => void deleteChart()} type="button">DELETE</button>}<button className="button" disabled={saving} onClick={() => { loadConfig(cloneDefault({ id: makeId() })); setMessage("New chart draft."); }} type="button">RESET</button><button className="button button-primary save-button" disabled={saving || !config.audioUrl || !config.title.trim() || typeof exactBpm !== "number" || !Number.isFinite(exactBpm) || exactBpm <= 0} onClick={() => void save()} type="button">{saving ? "SAVING…" : "SAVE CHART"}</button></div></footer>
