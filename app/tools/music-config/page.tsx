...
export default function MusicConfigPage() {
  ...
  const previewAnchor = (anchor: WaveformMarker) => {
    ...
    setMessage(`Previewing 5s before Beat ${anchor.beatIndex}.`);
  };
  ...
  return (
    <main className={`music-config-page theme-${theme} ${dockExpanded ? "audio-dock-expanded" : "audio-dock-collapsed"}`} style={pageStyle}>
      ...
      <header className="config-header">
        ...
        <div className="header-actions"><button className="button theme-toggle" onClick={() => setTheme(current => current === "dark" ? "light" : "dark")} type="button">{theme === "dark" ? "☀ LIGHT" : "☾ DARK"}</button></div>
      </header>

      <section className="charts-card section-card">
        <div className="section-heading"><div><span className="eyebrow">MUSIC CHARTS</span><h2>Chart library</h2></div><div className="chart-library-actions"><span className="count-badge">{library.length}</span><button className="button button-primary" onClick={() => setAddOpen(true)} type="button">＋ ADD / CHANGE AUDIO</button></div></div>
