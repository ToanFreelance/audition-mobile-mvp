...
async function uploadLocalAudio(file: File) {
  ...
  const response = await fetch(uploadUrl, {
    method: "POST",
    headers: { apikey: key, Authorization: `Bearer ${key}`, "Content-Type": file.type || "application/octet-stream", "x-upsert": "false", "cache-control": "3600" },
    body: file,
  });
  if (!response.ok) {
...
export default function MusicConfigPage() {
  ...
  const manualShiftSpace = (deltaMs: number) => patch("spaceStartMs", Math.max(0, Math.round(config.spaceStartMs + deltaMs)));
  ...
  const chooseStorageFile = (file: StorageAudioFile) => {
    ...
    next.durationMs = 0;
    setLocalFile(null); setAnalysis(null); setSelectedCandidate(null); setConfig(next); resetAudioState();
  ...
  const chooseLocalFile = (file: File | undefined) => {
    ...
    next.BPM_exact = undefined;
    next.durationMs = 0;
    setLocalFile(file); setAnalysis(null); setSelectedCandidate(null); setConfig(next); resetAudioState();
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!Number.isFinite(latest.BPM_exact) || (latest.BPM_exact ?? 0) <= 0) throw new Error("Chưa có BPM_exact. Hãy ANALYZE lại audio.");
      const response = await fetch("/api/music-config", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(latest) });
      ...
      setConfig(saved); setLocalFile(null); await refresh(); setMessage("Đã lưu chart.");
      setSaveDialog({ title: "Chart saved", message: uploaded ? "Audio đã upload và chart đã lưu." : "Chart đã lưu vào Music library.", audioUrl: saved.audioUrl });
    } catch (error) {
  ...
  const deleteTrack = async (track: MusicConfig) => {
    ...
    try {
      ...
      if (config.id === track.id) {
        ...
        setConfig(fallback ?? cloneDefault()); setAnalysis(null); setSelectedCandidate(null); resetAudioState();
      }
  ...
  return (
    <main className="music-config-page">
      ...
      <header className="config-header"><div><span className="eyebrow">CLUB AUDITION / CHART STUDIO</span><h1>Music Chart Config</h1><p>Analyze the track, verify the beat anchor, then save.</p></div><button className="primary header-add" disabled={saving} onClick={() => fileInputRef.current?.click()}>＋ ADD MUSIC</button></header>
      ...
      <div className="studio-layout">
        <aside className="config-panel library-panel">
          ...
          <div className="library-list">{library.length === 0 && <div className="empty-state">Chưa có chart. Chọn audio bên dưới hoặc ADD MUSIC.</div>}{library.map(track => <div key={track.id} className={`library-track ${config.id === track.id ? "active" : ""}`}><button className="library-select" disabled={saving} onClick={() => chooseTrack(track)}><span className="track-cover">♫</span><span className="track-copy"><b>{track.title}</b><small>{track.bpm || "—"} BPM · {Number.isFinite(track.BPM_exact) ? `${track.BPM_exact.toFixed(2)} exact` : "needs analysis"}</small></span></button><button className="library-delete" disabled={saving} onClick={() => void deleteTrack(track)} aria-label={`Xóa ${track.title}`}>×</button></div>)}</div>
          <div className="storage-block"><div className="storage-title"><span>STORAGE AUDIO</span><b>{storageFiles.length}</b></div><div className="storage-files">{storageFiles.length === 0 && <small>Chưa tìm thấy file audio.</small>}{storageFiles.map(file => { const configured = library.some(track => track.audioUrl === file.publicUrl); const active = currentStorageUrl === file.publicUrl; return <button key={file.publicUrl} className={`storage-file ${active ? "active" : ""}`} disabled={saving} onClick={() => chooseStorageFile(file)}><span>♫</span><span><b>{file.name}</b><small>{configured ? "Configured" : "Ready to chart"}{file.size ? ` · ${formatBytes(file.size)}` : ""}</small></span></button>; })}</div></div>
        </aside>
        ...
        <section className="config-panel editor-panel">
          ...
          <audio ref={audioRef} src={config.audioUrl} preload="metadata" onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onEnded={() => { setPlaying(false); previewEndRef.current = null; }} onTimeUpdate={() => { const audio = audioRef.current; if (!audio) return; const ms = audio.currentTime * 1000; setCurrentMs(ms); const endMs = previewEndRef.current; if (endMs != null && ms >= endMs) { previewEndRef.current = null; audio.pause(); audio.currentTime = endMs / 1000; setCurrentMs(endMs); } }} onLoadedMetadata={event => { const duration = event.currentTarget.duration; if (Number.isFinite(duration)) patch("durationMs", Math.round(duration * 1000)); }} />
          <div className="player-card"><button className="play-button" onClick={() => void togglePlay()}>{playing ? "❚❚" : "▶"}</button><div className="timeline-wrap"><input className="timeline" min="0" max={Math.max(config.durationMs, 1)} value={Math.min(currentMs, Math.max(config.durationMs, 1))} type="range" onChange={event => seek(Number(event.target.value))} /><div className="timeline-labels"><span>{formatTime(currentMs)}</span><span>{formatTime(config.durationMs)}</span></div></div><button onClick={() => seek(currentMs - 5000)}>−5s</button><button onClick={() => seek(currentMs + 5000)}>+5s</button></div>

          <div className="section-card analysis-card"><div className="section-head"><div><span>1 · TEMPO</span><h3>Auto analysis</h3></div><button className="primary" disabled={analyzing || saving || !config.audioUrl} onClick={() => void analyzeAudio()}>{analyzing ? "ANALYZING…" : "ANALYZE AUDIO"}</button></div><div className="tempo-summary"><div><span>Display</span><strong>{config.bpm > 0 ? config.bpm : "—"} <em>BPM</em></strong></div><div><span>System BPM exact</span><strong>{exactLabel}</strong></div><div><span>Confidence</span><strong>{analysis ? `${Math.round(analysis.confidence * 100)}%` : "—"}</strong></div></div>{analysis && <div className="candidate-list"><div className="candidate-head"><span>Detected candidates</span><small>ranked tempo clusters</small></div>{analysis.candidates.map((candidate, index) => <button key={`${candidate.bpm}-${index}`} className={`candidate-row ${index === 0 ? "recommended" : ""}`} onClick={() => { patch("BPM_exact", candidate.bpm); patch("bpm", Math.round(candidate.bpm)); setMessage(`Đã chọn ${candidate.bpm.toFixed(4)} BPM.`); }}><span className="candidate-rank">{index === 0 ? "★" : String(index + 1)}</span><span><b>{candidate.bpm.toFixed(4)} BPM</b><small>{candidate.source} · {Math.round(candidate.confidence * 100)}% signal</small></span>{index === 0 && <strong>RECOMMENDED</strong>}</button>)}</div>}<p className="helper">Không cần biết BPM trước. Hệ thống tự đọc audio, lấy beat timestamps và fit lại tempo; BPM hiển thị được round cho người chơi.</p></div>

          <div className="section-card timing-card"><div className="section-head"><div><span>2 · SPACE START</span><h3>Choose the 4-beat anchor</h3></div><button onClick={usePlayerTime}>USE PLAYER TIME</button></div><div className="anchor-current"><div><span>Current Space Start</span><strong>{formatTime(config.spaceStartMs)}</strong></div><div className="anchor-shifts"><button onClick={() => manualShiftSpace(-10)}>−10ms</button><button onClick={() => manualShiftSpace(-1)}>−1ms</button><button onClick={() => manualShiftSpace(1)}>+1ms</button><button onClick={() => manualShiftSpace(10)}>+10ms</button></div></div>{analysis ? <div className="anchor-grid"><div className="candidate-head"><span>Detected 4-beat anchors</span><small>Chọn mốc → slider về Perfect</small></div>{anchors.map((anchorMs, index) => <div key={anchorMs} className={`anchor-row ${selectedCandidate === anchorMs || config.spaceStartMs === anchorMs ? "active" : ""}`}><button onClick={() => selectAnchor(anchorMs)}><span>{index === 0 ? "★" : "○"}</span><b>{formatTime(anchorMs)}</b><small>4-beat #{index + 1}</small></button><button className="preview-button" onClick={() => void previewAnchor(anchorMs)}>▶ −5s</button></div>)}</div> : <div className="analysis-placeholder"><span>○</span><div><b>Chưa có beat grid</b><small>ANALYZE AUDIO để tìm tempo và mốc 4-beat.</small></div></div>}<p className="helper">Preview phát 5 giây trước anchor và tự dừng tại đúng mốc. Cách chỉnh thủ công bằng player + ±1/±10ms vẫn được giữ nguyên.</p></div>

          <div className="section-card gauge-card"><div className="section-head"><div><span>3 · GAUGE</span><h3>Timing preview</h3></div><span className="status-chip">4 BEAT CYCLE</span></div><div className="gauge-shell"><AuditionGauge bpm={timingBpm} value={previewGauge} animationDelayMs={previewAnimationDelayMs} zoneStart={config.gauge.zoneStartPercent} zoneEnd={config.gauge.zoneEndPercent} perfectStart={config.gauge.perfectStartPercent} perfectEnd={config.gauge.perfectEndPercent} /></div><div className="gauge-meta"><span>Anchor <b>{formatTime(config.spaceStartMs)}</b></span><span>Exact <b>{timingBpm.toFixed(4)} BPM</b></span><span>Phase <b>{formatTime(((currentMs - config.spaceStartMs) % previewCycleMs + previewCycleMs) % previewCycleMs)}</b></span></div></div>
