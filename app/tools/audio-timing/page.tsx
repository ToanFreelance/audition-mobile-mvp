...
export default function AudioTimingLabPage() {
  ...
  const setCurrentMode = (next: Mode) => { modeRef.current = next; setMode(next); };
  ...
  const getOutputStamp = (context: AudioContext): OutputStamp => {
    const withTimestamp = context as AudioContext & { getOutputTimestamp?: () => { contextTime?: number; performanceTime?: number } };
    ...
    return { contextTime: typeof contextValue === "number" && Number.isFinite(contextValue) ? contextValue : context.currentTime, performanceTime: typeof performanceValue === "number" && Number.isFinite(performanceValue) ? performanceValue : null };
  };
  ...
  const forcePlaybackAudioSession = () => {
    ...
    if (!session) { setAudioSessionType("unsupported"); return false; }
    try { session.type = "playback"; setAudioSessionType(session.type || "playback"); return true; }
    catch (error) { setAudioSessionType(`error: ${error instanceof Error ? error.message : "unknown"}`); return false; }
  };
  ...
  const tick = () => {
    ...
    if (context) {
      const stamp = getOutputStamp(context);
      setContextTime(context.currentTime); setOutputContextTime(stamp.contextTime); setOutputPerformanceTime(stamp.performanceTime);
  ...
  useEffect(() => {
    ...
    fetch("/api/music-config", { cache: "no-store" }).then(async response => {
      const json = await response.json(); if (!response.ok) throw new Error(json?.error || `HTTP ${response.status}`);
      const list: MusicConfig[] = Array.isArray(json) ? json : json.configs ?? []; if (cancelled) return;
      setConfigs(list); const aloha = list.find(item => item.title.toLowerCase().includes("aloha")); setSelectedId((aloha ?? list[0])?.id ?? "");
    }).catch(error => !cancelled && setMessage(`Load failed: ${error instanceof Error ? error.message : "unknown"}`)).finally(() => !cancelled && setLoading(false));
    return () => { cancelled = true; };
  }, []);

  useEffect(() => () => { stopRaf(); try { sourceRef.current?.stop(); } catch {} sourceRef.current?.disconnect(); void contextRef.current?.close(); }, []);
  ...
  useEffect(() => {
    ...
    const prepare = async () => {
      try {
        setReadyToTest(false); setDecoding(true); setMessage("Preparing AudioBuffer before the timing test…");
        try { sourceRef.current?.stop(); } catch {} sourceRef.current?.disconnect(); sourceRef.current = null; runRef.current = null; webPausedOffsetRef.current = 0;
        setCurrentMode("idle"); setWebTime(0); setNativeTime(0); setScheduledContextTime(null); setMarks([]); bufferRef.current = null;
  ...
  const stopSourceOnly = () => { try { sourceRef.current?.stop(); } catch {} sourceRef.current?.disconnect(); sourceRef.current = null; runRef.current = null; };

  const pauseAll = () => {
    const previousMode = modeRef.current;
    if (previousMode === "webaudio") { const captured = readWebSongTime(); webPausedOffsetRef.current = captured; setWebTime(captured); }
    nativeRef.current?.pause(); stopSourceOnly(); stopRaf(); setCurrentMode("idle");
    setMessage(`Paused · Native ${fmt(nativeRef.current?.currentTime ?? 0)} · WebAudio ${fmt(webPausedOffsetRef.current)}. PLAY WEBAUDIO resumes from this position; RESET returns to zero.`);
  };

  const stopForModeSwitch = () => {
    nativeRef.current?.pause();
    stopSourceOnly(); stopRaf(); setCurrentMode("idle");
  };

  const playNative = async () => {
    const audio = nativeRef.current; if (!audio || !readyToTest) return; stopForModeSwitch();
    try { audio.muted = false; audio.volume = 1; await audio.play(); setCurrentMode("native"); setMessage("Native playing · tap MARK BEAT on target beat."); rafRef.current = requestAnimationFrame(tick); }
    catch (error) { setMessage(`Native play failed: ${error instanceof Error ? error.message : "unknown"}`); }
  };

  const playWebAudio = async () => {
    if (!readyToTest) return; stopForModeSwitch(); const sessionForced = forcePlaybackAudioSession();
    try {
      const context = contextRef.current; const buffer = bufferRef.current; if (!context || !buffer) throw new Error("Prepared AudioBuffer is unavailable.");
      if (context.state !== "running") await context.resume(); updateLatencyInfo(context);
      const source = context.createBufferSource(); source.buffer = buffer; source.connect(context.destination);
      const offset = Math.min(webPausedOffsetRef.current, Math.max(0, buffer.duration - 0.001)); const when = context.currentTime + 0.10;
      runRef.current = { startContextTime: when, offset }; setScheduledContextTime(when); source.start(when, offset); sourceRef.current = source;
      setWebTime(offset); setCurrentMode("webaudio"); setMessage(`WebAudio playing FROM ${fmt(offset)} · audioSession=${sessionForced ? "playback" : getAudioSession()?.type ?? "unsupported"}.`);
      rafRef.current = requestAnimationFrame(tick);
      source.onended = () => { if (sourceRef.current === source) { sourceRef.current = null; runRef.current = null; stopRaf(); setCurrentMode("idle"); } };
    } catch (error) { setMessage(`Web Audio failed: ${error instanceof Error ? error.message : "unknown"}`); }
  };

  const markBeat = () => {
    const activeMode = modeRef.current;
    if (activeMode === "native") { const seconds = nativeRef.current?.currentTime ?? 0; setNativeTime(seconds); setMarks(current => [...current, { mode: "native", seconds }]); setMessage(`MARK native ${fmt(seconds)}.`); return; }
    if (activeMode === "webaudio") { const seconds = readWebSongTime(); setWebTime(seconds); setMarks(current => [...current, { mode: "webaudio", seconds }]); setMessage(`MARK WebAudio ${fmt(seconds)}.`); }
  };
...
function Clock({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) { return <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "baseline", borderBottom: "1px solid #2c2730", paddingBottom: 10 }}><span style={{ color: strong ? "#d969ff" : "#a9a0ae", fontSize: 12, fontWeight: 800 }}>{label}</span><strong style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", fontSize: strong ? 24 : 18 }}>{value}</strong></div>; }
function buttonStyle(primary: boolean): React.CSSProperties { return { minHeight: 52, borderRadius: 13, border: primary ? "1px solid #e278ff" : "1px solid #4a414f", background: primary ? "linear-gradient(135deg,#c62af0,#6f35df)" : "#1b1820", color: "white", fontWeight: 800, fontSize: 14 }; }
