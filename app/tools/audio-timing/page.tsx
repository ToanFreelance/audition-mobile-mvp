...
export default function AudioTimingLabPage() {
  ...
  const getOutputContextTime = (context: AudioContext): number => {
    ...
    return typeof timestampContextTime === "number" && Number.isFinite(timestampContextTime) ? timestampContextTime : context.currentTime;
  };
  ...
  const tick = () => {
    ...
    if (context) {
      ...
      if (mode === "webaudio") setWebTime(webStartOffsetRef.current + Math.max(0, outputCtx - webStartContextRef.current));
    }
  ...
  useEffect(() => {
    ...
    fetch("/api/music-config", { cache: "no-store" })
      .then(async response => {
        ...
        setMessage("Fresh test: PLAY WEBAUDIO first. iOS audioSession will be forced to playback on that tap.");
      })
  ...
  useEffect(() => {
    ...
    setWebTime(0); setNativeTime(0); setMode("idle");
  ...
  const stopAll = () => {
    ...
    try { sourceRef.current?.stop(); } catch {}
    sourceRef.current?.disconnect(); sourceRef.current = null; stopRaf(); setMode("idle");
  ...
  const playNative = async () => {
    ...
    try {
      ...
      await audio.play();
      setMode("native");
      setMessage("Native HTMLAudioElement is playing.");
      rafRef.current = requestAnimationFrame(tick);
  ...
  const playWebAudio = async () => {
    ...
    try {
      ...
      webStartOffsetRef.current = offset; webStartContextRef.current = when;
      source.start(when, offset); sourceRef.current = source;
      setWebTime(offset); setMode("webaudio");
      setMessage(`Web Audio playing · audioSession=${sessionForced ? "playback" : getAudioSession()?.type ?? "unsupported"}. Pause on Beat-4 and read OUTPUT SONG TIME.`);
      rafRef.current = requestAnimationFrame(tick);
      source.onended = () => { if (sourceRef.current === source) { sourceRef.current = null; setMode("idle"); stopRaf(); } };
    } catch (error) {
      setDecoding(false);
  ...
  const reset = () => {
    stopAll(); webPausedOffsetRef.current = 0; setWebTime(0);
    const native = nativeRef.current; if (native) native.currentTime = 0;
    setNativeTime(0); setMessage("Reset to 0:00. Refresh before the definitive fresh-start test.");
  };
  ...
  return <main style={{ minHeight: "100vh", padding: 24, background: "#111016", color: "#f7f3fb", fontFamily: "system-ui, sans-serif" }}>
    <div style={{ maxWidth: 760, margin: "0 auto" }}>
      ...
      <p style={{ color: "#aaa1b0", lineHeight: 1.5 }}>On iOS the lab forces navigator.audioSession.type=&quot;playback&quot; before Web Audio starts, so Silent/Ringer mode should not mute the AudioContext.</p>
      ...
      <section style={{ display: "grid", gap: 12, padding: 18, border: "1px solid #3a3340", borderRadius: 18, background: "#18151c" }}>
        <Clock label="NATIVE currentTime" value={fmt(nativeTime)} /><Clock label="WEBAUDIO OUTPUT SONG TIME" value={fmt(webTime)} strong /><Clock label="AudioContext.currentTime" value={contextTime ? fmt(contextTime) : "—"} /><Clock label="getOutputTimestamp().contextTime" value={outputContextTime != null ? fmt(outputContextTime) : "—"} /><Clock label="audioSession.type" value={audioSessionType} /><Clock label="baseLatency" value={baseLatency != null ? `${(baseLatency * 1000).toFixed(1)} ms` : "—"} /><Clock label="outputLatency" value={outputLatency != null ? `${(outputLatency * 1000).toFixed(1)} ms` : "—"} />
      </section>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 16 }}><button onClick={() => void playWebAudio()} disabled={decoding || mode !== "idle"} style={buttonStyle(true)}>{decoding ? "DECODING…" : "▶ PLAY WEBAUDIO"}</button><button onClick={() => void playNative()} disabled={decoding || mode !== "idle"} style={buttonStyle(false)}>▶ PLAY NATIVE</button><button onClick={stopAll} disabled={mode === "idle"} style={buttonStyle(false)}>Ⅱ PAUSE</button><button onClick={reset} disabled={mode !== "idle"} style={buttonStyle(false)}>RESET 0:00</button></div>
      <p style={{ marginTop: 18, padding: 14, borderRadius: 12, background: "#211a26", color: "#d7c9df", lineHeight: 1.45 }}>{message}</p>
      <p style={{ color: "#8f8795", fontSize: 13, lineHeight: 1.5 }}><strong>Test:</strong> refresh → PLAY WEBAUDIO first → do not touch Native → confirm audioSession.type says playback → pause on known Beat-4.</p>
    </div>
  </main>;
}
